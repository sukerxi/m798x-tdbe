...
## 🔧 固件说明
针对 Tenda BE12 Pro 的硬件特性及使用场景，对默认配置进行了以下针对性调整：

移除了该设备硬件不支持或非必要的功能包，以减小固件体积并降低内存占用：
- **❌ 移除 USB 相关支持**：由于该设备**无 USB 接口**，移除了 `kmod-usb-*`, `usbutils`, `block-mount` 等所有 USB 存储及外设驱动。
- **❌ 移除多余文件系统**： 移除了非必要的文件系统支持
- **❌ 移除MWAN相关支持**
- **🚫 默认禁用 802.11k (Radio Resource Management)**
...
| **5252pt** | [OpenWrt PR #21461](https://github.com/openwrt/openwrt/pull/21461) | 推动 Tenda BE12 Pro 进入 OpenWrt 主线 |